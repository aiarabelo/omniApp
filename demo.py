...
class Cookie(Base): #must inherit from the base object 
	...
	__tablename__ = "cookies" #Tells us where in the database we're going to store it, must define a table name 

	cookie_id = Column(Integer, primary_key = True) #Must have one or more columns with primary_key
	cookie_name = Column(String(50), index = True) #Don't put nothing in the string paranthesis; dangerous and risky otherwise

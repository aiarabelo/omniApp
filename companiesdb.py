...
class CompanyJobs:
    ...
    def insertJobListings(self, company_name, commitment, department, location, team, title, apply_url):
        conn = psycopg2.connect(host = credentials["DATABASE_HOST"], database = credentials["DATABASE_NAME"], user = credentials["DATABASE_USER"], password = credentials["DATABASE_PASSWORD"])
        cursor = conn.cursor()

        cursor.execute("""INSERT INTO job_listings (company_name, commitment, department, location, team, title, apply_url) VALUES (%s, %s, %s, %s, %s, %s, %s)""", (company_name, commitment, department, location, team, title, apply_url))

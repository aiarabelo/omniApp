...
if not IS_WIN:
    ...
    def _get_installed_pythons_for_view(bitness, view):
        ...
        for key in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
            try:
                python_core = winreg.OpenKey(
                    key, "Software\\Python\\PythonCore", 0, view | winreg.KEY_READ
                )
            except WindowsError:
            ...
            while True:
                try:
                    ...
                    i += 1
                    try:
                        at_path = winreg.QueryValue(
                            python_core, "{}\\InstallPath".format(version)
                        )
                    except WindowsError:
                        continue
...
class Logger(object):
    ...
    def start_progress(self, msg):
        ...
        ), "Tried to start_progress({!r}) while in_progress {!r}".format(
...
def main():
    ...
    if interpreter:
        ...
        if interpreter == sys.executable:
            ...
            if file.endswith(".pyc"):
                ...
                file = HERE
            sub_process_call = subprocess.Popen(
                [interpreter, file] + sys.argv[1:], env=env
            )
    ...
    if len(args) > 1:
        print(
            "There must be only one argument: DEST_DIR (you gave {})".format(
                " ".join(args)
            )
        )
        parser.print_help()
...
def install_wheel(project_names, py_executable, search_dirs=None, download=False):
    ...
    with search_dirs_context() as search_dirs:
        _install_wheel_with_search_dir(
            download, project_names, py_executable, search_dirs
        )
...
def change_prefix(filename, dst_prefix):
    ...
    if IS_DARWIN:
        prefixes.extend(
            (
                ...
                os.path.join(
                    "~", ".local", "lib", "python", sys.version[:3], "site-packages"
                ),
    ...
    assert False, "Filename {} does not start with any of these prefixes: {}".format(
        filename, prefixes
    )
    ...
    for modname in REQUIRED_MODULES:
        ...
        if filename is None:
            ...
            if (
                ...
                dst_filename = join(
                    dst_prefix, "lib", "python{}".format(sys.version[:3]), "readline.so"
                )
...
def install_python(
    home_dir, lib_dir, inc_dir, bin_dir, site_packages, clear, symlink=True
):
    ...
    if platform_include_dir != standard_lib_include_dir:
        platform_include_dest = distutils.sysconfig.get_python_inc(
    ...
    if sys.executable != py_executable:
        ...
        if IS_WIN or IS_CYGWIN:
            ...
            if os.path.exists(python_w):
                ...
                shutil.copyfile(
                    python_w,
                    os.path.join(os.path.dirname(py_executable), "pythonw.exe"),
                )
            ...
            if IS_PYPY:
                ...
                py_executable_dll_s = [
                    (
                        "python{}.dll".format(sys.version_info[0]),
                        "python{}_d.dll".format(sys.version_info[0]),
                    ),
                    (
                        "python{}{}.dll".format(
                            sys.version_info[0], sys.version_info[1]
                        ),
                        "python{}{}_d.dll".format(
                            sys.version_info[0], sys.version_info[1]
                        ),
                    ),
                ]
            ...
            for py_executable_dll, py_executable_dll_d in py_executable_dll_s:
                python_dll = os.path.join(
                    os.path.dirname(sys.executable), py_executable_dll
                )
                python_dll_d = os.path.join(
                    os.path.dirname(sys.executable), py_executable_dll_d
                )
                python_dll_d_dest = os.path.join(
                    os.path.dirname(py_executable), py_executable_dll_d
                )
                if os.path.exists(python_dll):
                    logger.info("Also created %s", py_executable_dll)
                    shutil.copyfile(
                        python_dll,
                        os.path.join(os.path.dirname(py_executable), py_executable_dll),
                    )
                if os.path.exists(python_dll_d):
                    ...
                    logger.info(
                        "Removed %s as the source does not exist", python_dll_d_dest
                    )
    ...
    try:
        ...
        if e.errno == errno.EACCES:
            logger.fatal(
                "ERROR: The executable {} could not be run: {}".format(py_executable, e)
            )
            sys.exit(100)
        else:
            raise e
    ...
    if proc_stdout != norm_home_dir:
        ...
        logger.fatal(
            "ERROR: It thinks sys.prefix is {!r} (should be {!r})".format(
...
def install_files(home_dir, bin_dir, prompt, files):
    ...
    for name, content in files.items():
        ...
        content = content.replace(
            "__VIRTUAL_WINPROMPT__", prompt or "({}) ".format(virtualenv_name)
        )
...
def install_distutils(home_dir):
    ...
    writefile(
        os.path.join(distutils_path, "distutils.cfg"), DISTUTILS_CFG, overwrite=False
    )
...
def fix_lib64(lib_dir, symlink=True):
    ...
    if not [
        p
        for p in distutils.sysconfig.get_config_vars().values()
    ...
    assert os.path.basename(lib_dir) == "python{}".format(
        sys.version[:3]
    ), "Unexpected python lib dir: {!r}".format(lib_dir)
    lib_parent = os.path.dirname(lib_dir)
    top_level = os.path.dirname(lib_parent)
    lib_dir = os.path.join(top_level, "lib")
    lib64_link = os.path.join(top_level, "lib64")
    assert os.path.basename(lib_parent) == "lib", "Unexpected parent dir: {!r}".format(
        lib_parent
    )
...
def resolve_interpreter(exe):
    ...
    if not os.path.exists(exe):
        logger.fatal(
            "The path {} (from --python={}) does not exist".format(exe, orig_exe)
        )
        raise SystemExit(3)
    if not is_executable(exe):
        logger.fatal(
            "The path {} (from --python={}) is not an executable file".format(
                exe, orig_exe
            )
        )
        raise SystemExit(3)
...
def fixup_scripts(_, bin_dir):
    if IS_WIN:
        new_shebang_args = (
            "{} /c".format(os.path.normcase(os.environ.get("COMSPEC", "cmd.exe"))),
            "",
            ".exe",
        )
    else:
    ...
    shebang = "#!{}".format(
        os.path.normcase(
            os.path.join(
                os.path.abspath(bin_dir), "python{}".format(new_shebang_args[2])
            )
        )
    )
...
def fixup_pth_file(filename):
    ...
    for line in prev_lines:
        ...
        if (
            ...
            if line != new_value:
                logger.debug(
                    "Rewriting path {} as {} (in {})".format(line, new_value, filename)
                )
